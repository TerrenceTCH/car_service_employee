...
class MechanicBookingDetails extends StatefulWidget {
  const MechanicBookingDetails({Key? key, required this.booking}) : super(key: key);

  final bookingObject booking;


  @override
  State<MechanicBookingDetails> createState() => _MechanicBookingDetailsPageState();
}

class _MechanicBookingDetailsPageState extends State<MechanicBookingDetails> {
  ...
  Future<List<BookingStatus>> getAllBookingStatus() async {
    CollectionReference ref = FirebaseFirestore.instance.collection('BookingStatus');
    QuerySnapshot snapshot = await ref.get();
    if(snapshot.docs.length>0){
      List<BookingStatus> dataList = [];
      dataList = snapshot.docs.map((doc) => BookingStatus(doc['Status'], doc['Description'], doc['Indicator'])).toList();
      if(dataList.length > 0){
        return Future.value(dataList);
      }
    }
  ...
  Future<void> updateBooking() async {
    ...
    //check status and get new status
    List<BookingStatus> bookstatusList = await getAllBookingStatus();
    String oldStatusIndicator = '';
    String newStatus = '';
    Timestamp time = Timestamp.fromDate(DateTime.now());
    for(BookingStatus bs in bookstatusList){
      if(bs.Status.compareTo(widget.booking.Status) == 0){
        oldStatusIndicator = bs.Indicator;
      }
    }
    int temp = int.parse(oldStatusIndicator);
    temp++;
    //if last, clear service in serviceCentre
    if(temp < 7){
      oldStatusIndicator = temp.toString();
      for(BookingStatus bs in bookstatusList){
        if(bs.Indicator.compareTo(oldStatusIndicator) == 0){
          newStatus = bs.Status;
        }
      }
      ...
      final message = {
        'notification': {
          ...
          'body': 'Your booking status has been updated! Click to check for progress.',
        },
        'data': {
  ...
  Future<void> revokeBooking() async {
    //check status and get new status
    List<BookingStatus> bookstatusList = await getAllBookingStatus();
    String oldStatusIndicator = '';
    String newStatus = '';
    ...
    int temp = int.parse(oldStatusIndicator);
    //if already at new, dont let revoke
    if(temp == 1){

      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Unable to revoke! Already at first state.'),
          duration: Duration(seconds: 2),
        ),
      );
    }
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Revoke Successfully!'),
          duration: Duration(seconds: 2),
        ),
      );
    }
